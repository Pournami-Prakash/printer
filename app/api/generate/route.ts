...
function buildFallbackReceipt(text: string, details: string, mood: Mood, intensity: Intensity) {
  ...
  const templates: Record<Mood, Record<Intensity, { main: string; best: string; worst: string }>> = {
    ...
    guilt: {
      soft: {
        main: `So your title is "${contextLine || "busy for mysterious reasons"}" and yet "${taskLine}" is still sitting there. You do make avoidance sound strangely professional.`,
        best: `You do "${taskLine}" and sleep lighter.`,
        worst: `Tomorrow inherits "${taskLine}" too.`
      },
      brutal: {
        main: `You are a "${contextLine || "professional deflector"}" and it shows, because "${taskLine}" still is not done. I can see you are committed to your current level of follow-through.`,
        best: `You finish "${taskLine}" and stop embarrassing yourself.`,
        worst: `You carry "${taskLine}" into tomorrow too.`
      },
      unhinged: {
        main: `Please explain how someone with the title "${contextLine || "senior procrastination officer"}" still has "${taskLine}" on layaway. You remind me self-awareness is a subscription service you cancelled.`,
    ...
    goblin: {
      soft: {
        main: `Be serious. You are a "${contextLine || "part-time menace"}" and "${taskLine}" is still in witness protection. That is not a workflow. That is jazz improvisation with consequences.`,
        best: `You do "${taskLine}" and the goblin calms down.`,
        worst: `The chaos follows "${taskLine}" into tomorrow.`
      },
      brutal: {
        main: `Absolutely not. A "${contextLine || "freelance nonsense dealer"}" still has not finished "${taskLine}". You are making simple things sound expensive and spiritually complex.`,
        best: `You do "${taskLine}" and reclaim your brain.`,
        worst: `You keep marinating while "${taskLine}" waits.`
      },
      unhinged: {
        main: `I have reviewed the crime scene and learned your title is "${contextLine || "chief nonsense architect"}" which frankly tracks, because "${taskLine}" is still untouched. This is gremlin behavior with admin consequences and optional shame.`,
